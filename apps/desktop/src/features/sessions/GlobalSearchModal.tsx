...
import { navigateToWorkspace } from "../workspaces/workspace-navigation";
...
export function GlobalSearchModal({ onClose }: { onClose: () => void }) {
  ...
  async function openResult(item: SessionSearchResultItem) {
    ...
    setOpening(true);
    try {
      const result = await navigateToWorkspace({
        cwd: item.cwd,
        ...(!item.archived ? { sessionPath: item.sessionPath } : {}),
      });
      // Reveal the shared failure card and its retry action beneath this modal.
